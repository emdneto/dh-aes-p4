...
def topology():
    ...
    makeTerm(b1, title='eth3', cmd="bash -c 'tcpdump -i b1-eth3 -w pcaps/b1-eth3.cap;'")
    if arg[1] == 'controller':
    ...
    if arg[1] == 'no_crypto':
        ...
        makeTerm(h1, title='sender', cmd="bash -c 'python no_controller/sender.py;'")
    elif arg[1] == 'controller':
        ...
        if arg[2] == 'miss':
            makeTerm(h1, title='send-miss', cmd="bash -c 'python miss/sender_data_miss.py;'")
            makeTerm(c1, title='send-miss', cmd="bash -c 'python miss/sender_control_miss.py;'")
        elif arg[2] == 'dh':
            makeTerm(c1, title='send-dh', cmd="bash -c 'python dh/sender_control_dh.py;'")
        elif arg[2] == 'aes':
            makeTerm(c1, title='send-aes', cmd="bash -c 'python aes/sender_control_aes.py;'")
            makeTerm(h1, title='send-aes', cmd="bash -c 'python aes/sender_data_aes.py;'")
        elif arg[2] == 'no-controller':
            makeTerm(h1, title='send-aes', cmd="bash -c 'python test/sender_data_aes.py;'")
        elif arg[2] == 'test':
            makeTerm(c1, title='send-aes', cmd="bash -c 'python test/sender_control_aes.py;'")
            makeTerm(h1, title='send-aes', cmd="bash -c 'python test/sender_data_aes.py;'")
